...
function force(v) {
    if (is_thunk(v)) {
        if(thunk_has_memoized_value(v)){
            ...
            var val= force(evaluate(thunk_expression(v),
                              thunk_environment(v)));
            thunk_memoize(v,val);
        return val;
    }
    ...
    if (is_self_evaluating(stmt)) {
        ...
        return make_return_value(
            evaluate(return_statement_expression(stmt),
                     env));
    } else if(is_variable_assignment(stmt)){
        return set_variable_value(stmt.variable,stmt.value,env);
...
function setup_environment() {
    ...
    var sec_env = extend_environment(list("NaN","Infinity","undefined"),list(NaN,Infinity,undefined),initial_env);
    return sec_env;
}
